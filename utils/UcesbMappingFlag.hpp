...
inline auto UcesbMap2String(UcesbMap map) -> std::string
{
    switch (map)
    {
        ...
        default:
            return std::format("", std::bitset<UCESB_MAP_BITSIZE>(static_cast<UcesbMapUType>(map)).to_string());
    }
}

template <>
class std::formatter<UcesbMap>
{
  public:
    ...
    constexpr auto format(const UcesbMap& flag, FmtContent& ctn) const
    {
        switch (presentation_)
        {
            default:
            case 's':
                return format_to(ctn.out(), "{}", UcesbMap2String(flag));
            case 'b':
                return format_to(ctn.out(), "0x{:b}", static_cast<UcesbMapUType>(flag));
        }
    }

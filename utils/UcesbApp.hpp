...
template <typename StructType>
template <typename Fnt>
void App<StructType>::init(Fnt&& fnt)
{
    auto status_ok = 1;
...
template <typename StructType>
void App<StructType>::launch_server(std::string_view exe, std::vector<std::string> args)
{
...
template <typename StructType>
void App<StructType>::setup()
{
    auto is_struct_map_success = uint32_t{};
    std::cout << std::format("Setting up ucesb client...\n");
    if (ucesb_client_.setup(nullptr, 0, ucesb_client_struct_info_.Get(), &is_struct_map_success, sizeof(StructType)) ==
        0)
    {
